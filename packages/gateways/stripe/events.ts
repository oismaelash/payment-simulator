...
function getStripePayloadsDir(): string {
  return resolve(getRepoRoot(), "packages/gateways/stripe/payloads");
}

/**
 * Stripe gateway adapter
 * Supports any event that has a corresponding payload JSON file in:
 *   packages/gateways/stripe/payloads/{event}.json
 */
export class StripeAdapter implements GatewayAdapter {
  ...
  getSupportedEvents(): string[] {
    try {
      const dir = getStripePayloadsDir();
