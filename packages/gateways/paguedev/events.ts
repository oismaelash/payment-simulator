...
function getPagueDevPayloadsDir(): string {
  return resolve(getRepoRoot(), "packages/gateways/paguedev/payloads");
}

/**
 * Pague.dev gateway adapter
 * Supports any event that has a corresponding payload JSON file in:
 *   packages/gateways/paguedev/payloads/{event}.json
 */
export class PagueDevAdapter implements GatewayAdapter {
  ...
  private loadHeaders(): void {
    try {
      const headersPath = resolve(
        getRepoRoot(),
        "packages/gateways/paguedev/headers.json"
      );
  ...
  getSupportedEvents(): string[] {
    try {
      const dir = getPagueDevPayloadsDir();
