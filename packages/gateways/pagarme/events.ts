...
function getPagarmePayloadsDir(): string {
  return resolve(getRepoRoot(), "packages/gateways/pagarme/payloads");
}

/**
 * Pagar.me gateway adapter
 * Supports any event that has a corresponding payload JSON file in:
 *   packages/gateways/pagarme/payloads/{event}.json
 */
export class PagarmeAdapter implements GatewayAdapter {
  ...
  getSupportedEvents(): string[] {
    try {
      const dir = getPagarmePayloadsDir();
